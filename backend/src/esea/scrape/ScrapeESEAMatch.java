...
{
	static String MATCH_DOC_TITLE_PATTERN = "^Premium - Statistics - Match # ([0-9]+)$";
	private Pattern title_pattern = Pattern.compile(MATCH_DOC_TITLE_PATTERN);

	public Document fetch(String esea_match_id) throws ScrapeException
	{    
		Document result;
		String esea_match_url = "http://play.esea.net/index.php?s=stats&d=match&id=" + esea_match_id;
		result = ScrapePage.open(esea_match_url);
		return result;
	}

	public EseaMatch parse(Document doc) throws ScrapeException
	{
		Matcher m = this.title_pattern.matcher(doc.title());
		String esea_match_id;
		
		if(!m.matches() || m.groupCount() != 1 || (esea_match_id = m.group(1)) == null || esea_match_id.isEmpty())
			throw new ScrapeException("attempting to parse document that is not an esea match: " + doc.title());

		EseaMatch result = null;
		...
		if(status_str.contains("Forfeit") || status_str.contains("Overturn"))
		{
